   ...
   <div class="comment">
      ...
      <g:if test="${comment instanceof IssueComment}">
         <g:if test="${comment.action == 'assign' && comment.relatedUser}">
            <strong><g:message code="comment.issue.${comment.action}"/></strong> la ${message(code: 'nectar.task.lower')}
            <g:if test="${comment.action == 'assign' && comment.relatedUser}">
   ...
   <div class="activity" style="clear: both;">
      ...
      <g:if test="${comment instanceof IssueComment}">
         <div>
            ...
            la ${message(code: 'nectar.task.lower')} <g:link controller="issues" action="view" id="${comment.issue.id}">${comment.issue.code}</g:link>
            <g:if test="${comment.action == 'assign' && comment.relatedUser}">
               a ${comment.relatedUser}
            </g:if>
